...
func (c *CertificatesController) Download() {
	...
	var data []byte
	existingPath := filepath.Join(clientsDir, name+".ovpn")
	if _, statErr := os.Stat(existingPath); statErr == nil {
		// Serve the pre-generated .ovpn (created by angristan or genclient.sh)
		var err error
		...
		// Fall back to regenerating from template
...
func (c *CertificatesController) saveClientConfig(keysPath string, name string) (string, error) {
	...
	// Per-client tls-crypt-v2 key (generated by genclient.sh alongside the cert).
	// Fall back to the server tls-crypt-v2.key or ta.key for legacy clients.
	taPath := filepath.Join(state.GlobalCfg.EasyRSAPath, "pki/private", name+"_tls_crypt_v2.key")
